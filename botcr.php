...
session_start();
require 'cauhinh2.php';
if(isset($_SESSION['botcr']) && isset($_POST['web']) && isset($_POST['page']) && isset($_POST['dbhost']) && isset($_POST['dbuser']) && isset($_POST['dbpass']) && isset($_POST['dbname'])){
  echo 'ok';
  $ConfigFile = fopen("cauhinh2.php", "w") or die("Không thể khởi tạo file này!");
  $ConfigContent = '<?php
...
<div class="row">
  ...
  <div class="form-group">
  <label for="title" class="col-sm-2 control-label">ID CRUSH</label>
  <div class="col-sm-10">
  <input type="text" class="form-control" id="idcr" name="idcr" placeholder="ID CRush" value="<?php echo $idcr ?>">
  <small>*hashtag này là để tránh j2team, bạn xóa nó nếu bạn muốn bot vào vào j2team hoặc chưa tham gia group</small>
  </div>
  </div>
  ...
  <div class="col-sm-10">
  <input type="text" class="form-control" id="camsuc" name="camsuc" placeholder="LIKE" value="<?php echo $camsuc ?>">
  ...
  <div class="col-sm-10">
  <input type="text" class="form-control" id="comment" name="comment" placeholder="CMT" value="<?php echo $comment ?>">
  ...
  <div class="col-sm-10">
  <input type="text" class="form-control" id="idkhongbot" name="idkhongbot" placeholder="100003880469096" value="<?php echo $idkhongbot ?>">
  ...
  <button id="botcr" type="button" class="btn btn-warning btn-fill">Cập nhật cấu hình</button>
  </div>
  </form>
<?php else: ?>
  <form class="form-horizontal" method="POST">
...
<script>
<?php if(isset($_SESSION['botcr'])): ?>
$( "#botcr" ).on( "click", function() {
  var web = $('#web').val()
  var page = $('#page').val()
  var hashtag = $('#hashtag').val()
  var camsuc = $('#camsuc').val()
  var comment = $('#comment').val()
  var bottt = $('#bottt').val()
  var idcr = $('#idcr').val()
  var idkhongbot = $('#idkhongbot').val()
  ...
  method: 'POST',
  url: 'botcr.php',
  data: { web: web, page: page, hashtag: hashtag, camsuc: camsuc, comment: comment, bottt: bottt, idcr: idcr, idkhongbot: idkhongbot, dbhost: dbhost, dbuser: dbuser, dbpass: dbpass, dbname: dbname },
  beforeSend: function () {
    $("#loading").show()
  },
